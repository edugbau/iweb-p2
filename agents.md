...
- **UI**: TailwindCSS (Modern Glassmorphism Style), Font Awesome.
...
1.  **Containerization**: Everything must be dockerized (`Dockerfile` required for all services, including Frontend, to support local development via Docker Compose, even if production is on Vercel).
...
- **Endpoint**: `GET https://nominatim.openstreetmap.org/search?q={query}&format=json`
  > **Important**: You MUST include a valid `User-Agent` header (e.g., `AppName/1.0`) in your request, otherwise Nominatim will block it (403 Forbidden).
...
- **Library**: `google-auth` + `httpx` (Manual OAuth 2.0 Authorization Code Flow implementation)
  > **Note**: We use a manual implementation instead of `fastapi-sso` due to Pydantic V2 compatibility issues. Requires both `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET`.
- **Flow**:
  1.  Frontend redirects user to Backend Login Endpoint (`/api/v1/auth/login`).
  2.  Backend generates OAuth URL with state (CSRF protection) and redirects to Google.
  3.  Google redirects back to Backend Callback (`/api/v1/auth/callback?code=...&state=...`).
  4.  Backend exchanges authorization code for ID token, verifies it, and issues JWT.
  5.  Backend redirects to Frontend (`/auth/callback?token=...`) with JWT token.
...
5.  **Frontend (Vercel)**:
    ...
    - **Note**: Ensure your repository contains a valid `Dockerfile` for the frontend to satisfy the "Everything must be dockerized" rule for grading/local dev.
